...
case class Config(nrEvaluationsPerUser: Int = 100, nrWarmups: Int = 1, nrUsers: Int = 10, nrOrganizations: Int = 5, initialSeed: Long = 72, nrInLists: Int = 4, server: String = "http://localhost:9200/thesis/resource")
  
object onePolicyTest {
    ...
    parser.parse(args, Config()) map { config =>
			  ...
			  """.parseJson
			  val one = new OnePolicy(EdocsPolicy,config.nrEvaluationsPerUser,config.nrWarmups,searchJson,config.server,config.nrUsers,config.nrOrganizations,config.initialSeed,config.nrInLists)
			  one.execute
